...
handle_input (struct keyboard *kp)
{
	...
	while (1) {
		...
		if (ev.value == 1) {
			switch (ev.code) {
			case 44:
				XTestFakeKeyEvent (dpy, buttons[0],
						   True, CurrentTime);
				break;
			case 13:
				XTestFakeKeyEvent (dpy, buttons[1],
						   True, CurrentTime);
				break;
			case 119:
				XTestFakeKeyEvent (dpy, buttons[2],
						   True, CurrentTime);
				break;
			case 68:
				XTestFakeKeyEvent (dpy, buttons[3],
						   True, CurrentTime);
				break;
			}
		} else if (ev.value == 0) {
...
main (int argc, char **argv)
{
	...
	cap_p = cap_get_proc ();
	make_kbd ("/dev/input/by-id/usb-BTC_USB_Multimedia_Keyboard-event-kbd",
		  0);
	cap_clear (cap_p);
